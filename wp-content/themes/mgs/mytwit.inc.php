...
class MyTwit {
	...
	public function UpdateCache() {
		...
		) return $this->ErrorMessage = 'MyTwit client is not configured properly.';
	...
	*/
	public function linkURLs($text) {
		...
		$out=array( '<a target="_blank" href="$1"'.$targetAppend.'>$1</a> ', '<a target="_blank" href="http://$1" rel="nofollow">$1</a>' );
		$text = preg_replace($in,$out,$text);
		$text = preg_replace('/@([a-zA-Z0-9-_]+)/','@<a target="_blank" href="http://twitter.com/$1" rel="nofollow">$1</a>',$text);
		return $text;
	}
	...
	*/
	protected function intoRelativeTime($created_at) {
		...
		if (($seconds / 60 / 60 / 24) > 1) return 'hace '.$this->formatPlural(round($seconds / 60 / 60 / 24), ' día');
		elseif (($seconds / 60 / 60) > 1) return 'hace '.$this->formatPlural(round($seconds / 60 / 60), ' hora');
		else if (($seconds / 60 ) > 1) return 'hace '.$this->formatPlural(round($seconds / 60), ' minuto');
		else return 'hace '.round($seconds).' segundo';
	}
